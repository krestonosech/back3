<template>
    <div class="good">
      <h3 class="name">Яйца</h3>
      <img width="150px" height="150px" src='../assets/public/яйца.jpg' />
      <p class="description">Прекрасные Яйца</p>
      <p class="price">19 руб.</p>
      <div v-if="isAdd && number > 0" class="buttons">
        <button class="number" @click="number++; register('Яйца')">+</button>
        <p>{{ number }}</p>
        <button class="number" @click="number--; register('Яйца')">-</button>
      </div>
      <button v-else @click="register('Яйца'); number = 1">Добавить товар</button>
    </div>
  </template>
