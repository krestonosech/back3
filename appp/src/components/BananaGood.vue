<template>
    <div class="good">
      <h3 class="name">Банан</h3>
      <img width="150px" height="150px" src='../assets/public/банан.jpg' />
      <p class="description">Прекрасный Банан</p>
      <p class="price">19 руб.</p>
      <div v-if="isAdd && number > 0" class="buttons">
        <button class="number" @click="number++; register('Банан')">+</button>
        <p>{{ number }}</p>
        <button class="number" @click="number--; register('Банан')">-</button>
      </div>
      <button v-else @click="register('Банан'); number = 1">Добавить товар</button>
    </div>
  </template>
